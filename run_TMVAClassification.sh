...
#Define the NTrees values
nTrees=(50,100,200,400,600,800,1000)
##
# algo="BDT,BDTG,LD,DNN_GPU"
#algo="BDT,BDTG,BDTD,BDTF,BDTB"
algo="BDT"
...
#Loop over NTrees values and run training
for NTREES in "${nTrees[@]}"; do
	echo "Running TMVAClassification with NTrees=$NTREES..."
	./TMVAClassification.exe $inputs $inputb "$cuts" "$cutb" $outputmvadir "$algo" "$NTREES"
done
...
#After training, plot the ROC curves for different NTrees
echo "Plotting ROC curves for different NTrees..."
./guiefficiencies.exe $outputmvadir "$algo" "${nTrees[@]}"

#Clean up temporary executable files
rm TMVAClassification.exe guivariables.exe guiefficiencies.exe guieffvar.exe mvaprod.exe
